...
fn reading(label: &str, value: impl Into<String>) -> Reading {
    Reading { label: label.into(), value: value.into(), tone: String::new() }
}

fn toned(label: &str, value: impl Into<String>, tone: &str) -> Reading {
    Reading { label: label.into(), value: value.into(), tone: tone.into() }
}
...
fn compose(
    ...
    if let Some(entries) = diagnostics.and_then(|d| d.get("temperatures")).and_then(Value::as_array) {
        for entry in entries.iter().take(4) {
            let name = entry.get("name").and_then(Value::as_str).unwrap_or("sıcaklık");
            if let Some(celsius) = entry.get("celsius").and_then(Value::as_f64) {
                machine.push(toned(name, format!("{celsius:.0} °C"), heat_tone(celsius)));
            }
        }
    }
    groups.push(Group { title: "Makine".into(), rows: machine });

    // ---------------------------------------------------------------- storage
    let mut storage = Vec::new();
    if let Some(volumes) = diagnostics.and_then(|d| d.get("storage")).and_then(Value::as_array) {
    ...
    groups.push(Group { title: "Depolama".into(), rows: storage });
    ...
    groups.push(Group { title: "Ağ".into(), rows: network });
    ...
    let mut screen = vec![
        reading(
            "Kip",
            if width > 0 { format!("{width}×{height} @ {refresh} Hz") } else { dash() },
        ),
    ...
    if phases.frames > 0 {
        ...
        screen.push(reading("Sayfa çevirme beklemesi", format!("{:.1} ms", phases.flip_ms)));
    }
    groups.push(Group { title: "Ekran ve GPU".into(), rows: screen });
    ...
    let mut playback = vec![
        ...
        reading(
            "Ekranı tutan",
            display.and_then(|d| d.owner.clone()).unwrap_or_else(|| "boşta".into()),
        ),
    ];
    if let Some(sessions) = number(status, "/media/sessions") {
        playback.push(reading("Medya oturumu", format!("{sessions:.0}")));
    }
    groups.push(Group { title: "Oynatma".into(), rows: playback });
    ...
    if let Some(errors) = status.and_then(|s| s.pointer("/cec/errors")) {
        let sum: u64 = ["receive", "transmit", "nack", "low_drive", "arbitration_lost"]
            .iter()
            .filter_map(|key| errors.get(key).and_then(Value::as_u64))
            .sum();
        cec.push(toned("Hata", sum.to_string(), if sum == 0 { "good" } else { "warn" }));
    }
    groups.push(Group { title: "CEC".into(), rows: cec });

    // --------------------------------------------------------------- services
    let mut services = Vec::new();
    if let Some(entries) = status.and_then(|s| s.get("services")).and_then(Value::as_array) {
        for entry in entries {
            let name = entry.get("name").and_then(Value::as_str).unwrap_or("servis");
            let healthy = entry.get("healthy").and_then(Value::as_bool).unwrap_or(false);
            let detail = entry
                .get("detail")
                .and_then(Value::as_str)
                .filter(|s| !s.is_empty())
                .map(str::to_string)
                .unwrap_or_else(|| if healthy { "çalışıyor".into() } else { "durdu".into() });
    ...
    groups.push(Group { title: "Servisler".into(), rows: services });
...
mod tests {
    ...
    fn the_brief_s_groups_are_all_here() {
        let mut diagnostics = Diagnostics::new();
        diagnostics.compose(None, None, None);
        let titles: Vec<&str> = diagnostics.groups.iter().map(|g| g.title.as_str()).collect();
        for wanted in ["Makine", "Depolama", "Ağ", "Ekran ve GPU", "Oynatma", "CEC", "Servisler"] {
    ...
    fn a_hot_board_says_so() {
        ...
        let row = machine.rows.iter().find(|r| r.label == "soc").expect("soc reading");
