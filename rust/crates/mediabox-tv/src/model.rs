...
//!
//! Copied from the web interface's model rather than shared with it. Extracting
//! a common crate would mean rebuilding the WebAssembly bundle to prove nothing
//! broke, and that needs the wasm32 target and the exact wasm-bindgen the build
//! pins. The interface on the television is not worth risking the one on the
//! phone for; the two are reconciled once, later, on purpose.
//!
...
/// The appliance's own library, as the media core labels it.
...
impl MetaPreview {
    pub fn is_library(&self) -> bool {
        self.addon_id.as_deref() == Some(LIBRARY_ADDON_ID)
    }
}
