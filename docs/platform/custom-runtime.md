...
| `0012-gbm-signal-hdr-only-where-the-link-can-carry-it` | Signals HDR only where ten-bit RGB fits what the sink declared, at the mode currently set; where it does not, gives up BT.2020 and the tenth bit together so the link stays RGB and VOP2 converts | PRODUCTION_REQUIRED | A sink reports its sockets separately. A Sony KD-65XE9005 declares HDMI 1 a 300 MHz port and HDMI 3 a 600 MHz one; ten-bit RGB at 4K24 is 371 MHz. Over the ceiling the driver keeps the depth, gives up the chroma, transmits 4:2:2 -- and does not write that back to `color_format`, so `0006`'s live read cannot see it either. The tag then says BT.2020 RGB over YCbCr pixels and the television decodes them with the wrong matrix: HDR arrives visibly wrong on the slower socket and correctly on the faster one, from the same binary. A sink that declares no ceiling is left on RGB, because a missing byte is not evidence of a narrow link. Making the tag honest is not enough and was tried first: a 4:2:2 link renders HDR wrong on this driver whether it is labelled RGB or YCC, which is why the transport is what gets changed. Measured across both of that set's sockets on 2026-09-18/19: HDMI 1 at 4K now transmits BT.709 RGB 8-bit and is correct, HDMI 3 at 4K keeps HDR10 and is correct, and neither needs a setting. |
