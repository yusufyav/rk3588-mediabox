...
| `0012-gbm-ask-for-rgb-only-where-rgb-can-arrive` | Requests RGB only when ten-bit RGB fits what the sink declared it can receive, at the mode currently set | PRODUCTION_REQUIRED | A sink reports its sockets separately. A Sony KD-65XE9005 declares HDMI 1 a 300 MHz port and HDMI 3 a 600 MHz one; ten-bit RGB at 4K24 is 371 MHz. Over the ceiling the driver keeps the depth, gives up the chroma, transmits 4:2:2 -- and does not write that back to `color_format`, so `0006`'s live read cannot see it either. The tag then says BT.2020 RGB over YCbCr pixels and the television decodes them with the wrong matrix: HDR arrives visibly wrong on the slower socket and correctly on the faster one, from the same binary. A sink that declares no ceiling is left on RGB, because a missing byte is not evidence of a narrow link. |
